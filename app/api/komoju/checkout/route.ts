...
const PLANS: Record<string, { amount: number; description: string; cookieValue: string }> = {
  standard: { amount: 980, description: 'クレームAI スタンダードプラン（月額）', cookieValue: '1' },
  business: { amount: 2980, description: 'クレームAI ビジネスプラン（月額）', cookieValue: 'biz' },
}
...
export async function POST(req: Request) {
  try {
    ...
    const baseUrl = process.env.NEXT_PUBLIC_BASE_URL || 'https://claim-ai-beryl.vercel.app'
